CREATE FUNCTION get_order_invoice_order_name(order_id INT)
RETURNS text AS $$
DECLARE order_name varchar(50);
BEGIN
    SELECT order_details_view."Order name"
	INTO order_name
...
CREATE FUNCTION get_order_invoice_products(order_id INT)
RETURNS TABLE (
...
CREATE FUNCTION get_order_invoice_total_price(order_id INT)
RETURNS float AS $$
DECLARE
    total_sum float;
BEGIN
    SELECT CAST(SUM(order_details_view."Product price" * order_details_view."Product quantity") AS DECIMAL(10, 2))
    INTO total_sum
    FROM order_details_view
    WHERE order_details_view."Order ID" = order_id;
    RETURN total_sum;
